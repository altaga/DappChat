...
class MainMNM extends Component {
  constructor(props) {
    super(props);
    this.state = {
      account: null,
    ...
    autoBind(this);
    this.provider = new ethers.providers.JsonRpcProvider("https://polygon-mumbai.g.alchemy.com/v2/XXXXXXXXXXXXXXXXXXXXXXX")
    this.web3 = new Web3(new ethers.providers.JsonRpcProvider("https://polygon-mumbai.g.alchemy.com/v2/XXXXXXXXXXXXXXXXXXXXXXX"));
    this.wallet = null
  ...
  async componentDidMount() {
    ...
    if (!startFlag) {
      ...
      if (flag) {
        cookies.set('address', temp, { path: '/main/' });
        this.setState({
          addresses: temp,
          activeAddress: this.props.match.params.address,
      ...
      this.wallet = new ethers.Wallet(cookies.get('wallet'), this.provider)
      const account = this.wallet.address
      this.chatContract = new ethers.Contract(contractAddress, abi(), this.wallet);
      let balance = await getMaticBalance(account)
      this.setState({
        ewallet: cookies.get('ewallet'),
        account: {
          ethAddress: account,
  ...
  async syncNFT(address) {
    ...
    temp.forEach((item) => {
      if (item.contractAddress !== "") {
        contractsNFT.push(new ethers.Contract(item.contractAddress, abi2(), this.wallet));
      }
    })
    let res = []
                        ...
                        <button
                          ...
                          className='myButton' onClick={() => {
                            if (document.getElementById("addressValueInit0128").value.length === 42) {
                              window.open(`/main/${document.getElementById("addressValueInit0128").value}`, "_self")
                            }
                            else {
...
export default routerParams(withCookies(MainMNM));